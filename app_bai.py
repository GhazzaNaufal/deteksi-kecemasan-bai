...
def injeksi_css_kustom():
    ...
    [data-testid="stRadio"] {
        ...
        padding: 30px 40px; /* Jarak lega agar tidak terasa sesak */
        border-radius: 16px; 
        box-shadow: 0 4px 15px rgba(0,0,0,0.03); /* Bayangan sangat halus */
        border-top: 5px solid #457B9D; /* Aksen garis Biru Kalem di atas */
    ...
    [data-testid="stMetric"] {
        ...
        border-top: 5px solid #2A9D8F; /* Aksen Teal */
    }
    ...
    /* Tombol Utama warna Teal (Empatik dan Mendorong Rasa Aman) */
    .stButton>button {
...
def simpan_data(nama, status, umur, jk, pekerjaan, pendidikan, skor, tingkat):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    ...
    c.execute('''INSERT INTO riwayat 
                 ...
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
              (nama, status, umur, jk, pekerjaan, pendidikan, tanggal, skor, tingkat))
    conn.commit()
    conn.close()
...
if peran == "Pasien":
    ...
    if st.button("Lihat Hasil", type="primary"):
        if nama.strip() == "":
            ...
            simpan_data(nama, status, umur, jk, pekerjaan, pendidikan, skor_total, tingkat)
...
# HALAMAN ADMIN PSIKOLOG
    ...
    if password == "pakar123":
        ...
        if file_csv is not None:
            # Ditambah tombol agar data tidak otomatis ter-import berkali-kali (duplikat)
            if st.button("Proses Import Data", type="primary"):
                try:
                    df_import = pd.read_csv(file_csv)
                    kolom_wajib = ['nama', 'status', 'umur', 'jenis_kelamin', 'pekerjaan', 'pendidikan', 'tanggal', 'skor_bai', 'tingkat_cemas']
                    ...
                    if all(col in df_import.columns for col in kolom_wajib):
                        ...
                        st.error("Format kolom CSV tidak sesuai! Pastikan file CSV tersebut hasil ekspor dari aplikasi ini.")
        ...
        if not df.empty:
            ...
            st.subheader("Grafik Distribusi Tingkatan Kecemasan (BAI)")
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            st.subheader("Tabel Data Detail Pasien")
            ...
            with col_del1:
                opsi_pasien = [f"{row['id']} - {row['nama']}" for index, row in df.iterrows()]
                pilihan_hapus = st.selectbox("Daftar Pasien:", opsi_pasien, label_visibility="collapsed")
                
            with col_del2:
                if st.button("Hapus Pasien", type="primary", use_container_width=True):
                    if pilihan_hapus:
                        id_terpilih = int(pilihan_hapus.split(" - ")[0])
                        hapus_data_pasien(id_terpilih)
